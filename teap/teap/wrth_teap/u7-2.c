...
void solution(int depth) 
{
    ...
    for (int i = depth == getExtCenter(), remainder = 0; i < 10; i++) 
    {
        ...
        remainder = ((new_pow(10, getExtCenter() - depth) * i) % mod + ((new_pow(10, getCenter() + depth) * i) % mod)) % mod;
...
int main() 
{
    ...
    for (int i = 0; i < n; i++) 
    {
        ...
        for (int i = 0 == getExtCenter(); i < 10; i++) 
            arr[0][(((new_pow(10, getCenter()) * i) + (!(length % 2) ? (new_pow(10, getCenter() - 1) * i) : (0))) % mod)] += 1;
