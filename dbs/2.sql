...
join
	-- this join is here to make it faster by selecting only top5
	single_player_records spr on
	dtc.player_id = spr.player_id
...
join players p on
		ppg.player_id = p.id
join apg_tab apg on
	apg.player_id = ppg.player_id
	and apg.team_id = ppg.team_id
join teams t on
		ppg.team_id = t.id
join player_records_with_games_count prwgc
  on
